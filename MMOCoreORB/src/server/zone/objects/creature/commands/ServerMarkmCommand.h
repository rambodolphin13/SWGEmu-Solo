#ifndef SERVERMARKMCOMMAND_H_
#define SERVERMARKMCOMMAND_H_

#include <fstream>

class ServerMarkmCommand {
public:
	static int executeCommand(CreatureObject* creature, uint64 target, const UnicodeString& arguments) {
		...
		StringBuffer entry;
		entry << "{ role = \"musician\", planet = \"" << zone->getZoneName()
		...
		creature->sendSystemMessage("Musician location marked:");
	...
	static void sendSyntax(CreatureObject* player) {
		if (player != nullptr)
			player->sendSystemMessage("Syntax: /server markm");
	}
};

#endif // SERVERMARKMCOMMAND_H_
